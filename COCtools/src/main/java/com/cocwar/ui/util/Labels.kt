...
/** 从名称中提取年份（后两位） */
fun parseYearFromName(name: String): Int? {
    if (name.length < 7) return null
    return name.substring(1, 3).toIntOrNull()
}

/** 从名称中提取月份 */
fun parseMonthFromName(name: String): Int? {
    if (name.length < 7) return null
    return name.substring(3, 5).toIntOrNull()
}
